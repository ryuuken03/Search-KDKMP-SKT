...
const data = JSON.parse(fs.readFileSync('assets/combined/akhir/data.json', 'utf8'));

const chunksDir = 'assets/combined/akhir/chunks';
const namesDir = 'assets/combined/akhir/names';
...
for (let i = 0; i < data.length; i++) {
    ...
    // Array format for UI compatibility:
    // [Page_Number, No, Nomor_Kartu, Nama, Jabatan_Label, Pendidikan, Keterangan]
    const rowArray = [
        row.Page || 0,
        row.No,
...
fs.writeFileSync('assets/combined/akhir/no_peserta.json', JSON.stringify(noPeserta));
fs.writeFileSync('assets/combined/akhir/peringkat.json', JSON.stringify(peringkat));
