...
2. **Dashboard Ringkasan Instan**: Dashboard statistik dimuat seketika karena membaca file `assets/kdkmp/summary.json` kecil (250 bytes) saat halaman pertama dibuka.
...
4. **Bypass Batas Ukuran File Vercel**: File PDF asli (`assets/kdkmp/source.pdf`) tidak ikut disertakan ke dalam berkas build produksi, sehingga aman dari batas ukuran file Vercel Hobby (max 50MB).
...
1. Simpan berkas PDF Anda dengan nama `source.pdf` di dalam folder `assets/kdkmp/` (`assets/kdkmp/source.pdf`).
2. Jalankan script pengekstraksi data untuk mengubah PDF menjadi indeks JSON terkompresi:
   ```bash
   node scripts/extract_kdkmp.mjs
   ```
   *(Script ini akan membaca 12.473 halaman secara lokal dan menghasilkan file `assets/kdkmp/data.json` serta `assets/kdkmp/summary.json` secara otomatis).*
...
Perintah ini akan membuat folder `dist/` yang hanya berisi file static HTML/JS/CSS dan database JSON terkompresi. Berkas PDF asli (`assets/kdkmp/source.pdf`) **sengaja dilewatkan** agar tidak melebihi kuota upload dan membebani server hosting Anda.
