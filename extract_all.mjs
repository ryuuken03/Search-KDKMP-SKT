...
const pages = JSON.parse(fs.readFileSync('scratch_pdf_pages.json', 'utf8'));
    ...
    if (currentJabatan && currentJabatan.startParsing) {
        ...
        while ((match = rowRegex.exec(pageText)) !== null) {
            currentJabatan.dataRows.push({
                ...
                "Jabatan": match[4],
                "Kualifikasi Pendidikan": match[5],
...
jabatans.forEach(j => {
    ...
    finalSummary.jabatan[j.label] = {
        ...
        source: {
            ...
            jumlahPeserta: j.peserta.toString(), // use extracted peserta count if needed, or j.dataRows.length
            kehadiran: {
            ...
            kelulusan: {
                ...
                persen: ((lulusCount / j.dataRows.length) * 100).toFixed(2)
            },
...
fs.writeFileSync('assets/combined/akhir/summary.json', JSON.stringify(finalSummary, null, 2));
fs.writeFileSync('assets/combined/akhir/data.json', JSON.stringify(allData, null, 2));
